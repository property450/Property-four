...
            h.lat !== null &&
            h.lng !== null &&
            !isNaN(parseFloat(h.lat)) &&
            !isNaN(parseFloat(h.lng))
        );
        setProperties(valid);
      }
    }
...
    const distance = getDistance(center.lat, center.lng, parseFloat(house.lat), parseFloat(house.lng));
    return (
...
                      if (isFav) {
                        await supabase.from('favorites').delete().eq('user_id', user.id).eq('property_id', house.id);
                      } else {
                        await supabase.from('favorites').insert({ user_id: user.id, property_id: house.id });
                      }
